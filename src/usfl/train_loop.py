...
def train_dynamic_usfl(
    ...
    # ------------------------------
    # 3. LinUCB DualCut 初始化
    # ------------------------------
    img_size = int(getattr(cfg.data, "img_size", 32) if hasattr(cfg, "data") else 32)
    ...
    for epoch in range(1, cfg.training.epochs + 1):
        ...
        if val_loader is not None and (epoch % cfg.training.eval_every == 0):
            ...
            # 计算本轮动态代价并更新 LinUCB
            dynamic_cost = compute_dynamic_cost(cfg, epoch)
            linucb_dualcut.update(decision.action, dynamic_cost)
